...
describe('Resume Processing Integration Tests', () => {
    ...
    beforeAll(async () => {
        ...
        const testUser = await (0, testUserFactory_1.createVerifiedUserWithProfile)('resume-test@example.com', 'Password123!');
    ...
    describe('End-to-End Resume Upload and Processing', () => {
        it('should successfully upload and process a PDF resume', async () => {
            ...
            expect(skills.some(skill => skill.toLowerCase().includes('javascript') ||
                skill.toLowerCase().includes('python') ||
                skill.toLowerCase().includes('react'))).toBe(true);
        }, 30000);
        it('should handle resume processing with fallback when GPT fails', async () => {
            ...
            const testProfileService = new ProfileService_1.ProfileService(s3Service, failingOpenAIService, prisma_1.default);
            const testUser2 = await (0, testUserFactory_1.createVerifiedUserWithProfile)('resume-test2@example.com', 'Password123!');
            const testUserId2 = testUser2.id;
            try {
                const s3Key = await testProfileService.uploadResume(testUserId2, testPdfBuffer, 'test-resume-fallback.pdf');
                await new Promise(resolve => setTimeout(resolve, 5000));
                const profile = await prisma_1.default.userProfile.findUnique({
                ...
                await prisma_1.default.user.deleteMany({ where: { email: 'resume-test2@example.com' } });
            }
            catch (error) {
                await prisma_1.default.userProfile.deleteMany({ where: { userId: testUserId2 } });
                await prisma_1.default.user.deleteMany({ where: { email: 'resume-test2@example.com' } });
        ...
        it('should validate file size and reject oversized files', async () => {
            ...
            await expect(profileService.uploadResume(testUserId, oversizedBuffer, 'oversized.pdf')).rejects.toThrow(auth_1.ValidationError);
        });
        ...
        it('should replace old resume when uploading new one', async () => {
            const firstS3Key = await profileService.uploadResume(testUserId, testPdfBuffer, 'first-resume.pdf');
            await new Promise(resolve => setTimeout(resolve, 1000));
            const secondS3Key = await profileService.uploadResume(testUserId, testPdfBuffer, 'second-resume.pdf');
        ...
        it('should handle resume download for user without resume', async () => {
            const testUser3 = await (0, testUserFactory_1.createVerifiedUserWithProfile)('no-resume@example.com', 'Password123!');
            const testUserId3 = testUser3.id;
            try {
                await expect(profileService.getResumeUrl(testUserId3)).rejects.toThrow(auth_1.NotFoundError);
                await prisma_1.default.userProfile.deleteMany({ where: { userId: testUserId3 } });
                await prisma_1.default.user.deleteMany({ where: { email: 'no-resume@example.com' } });
            }
            catch (error) {
                await prisma_1.default.userProfile.deleteMany({ where: { userId: testUserId3 } });
                await prisma_1.default.user.deleteMany({ where: { email: 'no-resume@example.com' } });
    ...
    describe('Resume Text Extraction', () => {
        it('should extract text from PDF with technical skills', async () => {
            const s3Key = await profileService.uploadResume(testUserId, testPdfBuffer, 'skills-test.pdf');
            await new Promise(resolve => setTimeout(resolve, 5000));
            const profile = await prisma_1.default.userProfile.findUnique({
                where: { userId: testUserId }
            });
...
//# sourceMappingURL=resume-processing.integration.test.js.map